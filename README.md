...
for d in ['50', '100', '200', '300']:
    for n in ['none', 'abtt']:
        wbc.import_from_file(importpath+"glove.6B."+d+"d.txt", 
                             ...
                             prepro_picklefile="")
```
Using ```norm:abtt ``` creates a normalized version as described in <a href="https://arxiv.org/abs/1702.01417" target=_new>this</a> paper. Parameter D is set to ```D=max(int(dim/100), 1)```.
</p>
